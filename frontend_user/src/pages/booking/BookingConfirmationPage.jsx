...
import { useBooking, normalizeBookingResponse } from '../../contexts/BookingContext';
...
const BookingConfirmationPage = () => {
  const {
    ...
    hydrateBookingFromSnapshot,
    saveBookingSnapshot
  } = useBooking();
  const { currentUser } = useAuth();
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  const storedSnapshot = React.useMemo(() => getStoredBookingSnapshot?.(), [getStoredBookingSnapshot]);
  const [bookingDetails, setBookingDetails] = useState(() => bookingState.confirmedBooking || storedSnapshot?.booking || null);
  const [fetchError, setFetchError] = useState('');
  const [isLoading, setIsLoading] = useState(false);
  const bookingIdFromQuery = searchParams.get('bookingId');

  useEffect(() => {
    if (bookingState.bookingComplete || bookingDetails || storedSnapshot?.booking || bookingIdFromQuery) {
      return;
    }
    ...
    if (!bookingState.paymentMethod && !bookingState.lastPaymentMethod) {
      navigate('/booking/payment');
    }
  }, [
    bookingState.bookingComplete,
    bookingState.serviceType,
    bookingState.packageDuration,
    bookingState.date,
    bookingState.selectedSeat,
    bookingState.paymentMethod,
    bookingState.lastPaymentMethod,
    bookingDetails,
    storedSnapshot,
    bookingIdFromQuery,
    navigate
  ...
  useEffect(() => {
    if (bookingState.confirmedBooking) {
      setBookingDetails(bookingState.confirmedBooking);
      return;
    }
    if (storedSnapshot?.booking) {
      setBookingDetails(storedSnapshot.booking);
      hydrateBookingFromSnapshot?.(storedSnapshot);
      return;
    }
    const targetId = bookingIdFromQuery || bookingState.recentBookingId || storedSnapshot?.booking?.id;
    if (!targetId) return;
    ...
    const fetchBooking = async () => {
      ...
      try {
        ...
        const response = await fetch(`${apiUrl}/api/bookings/${targetId}`, {
          method: 'GET',
          headers: {
        ...
        if (!response.ok) {
          const message = response.status === 404
            ? 'Booking không tồn tại hoặc đã bị xoá.'
            : 'Không thể tải thông tin booking. Vui lòng thử lại.';
          throw new Error(message);
        }
        const data = await response.json();
        const normalized = normalizeBookingResponse(data.booking || {});
        if (!isMounted) return;
        const snapshot = { booking: normalized, paymentMeta: storedSnapshot?.paymentMeta, storedAt: Date.now() };
        hydrateBookingFromSnapshot?.(snapshot);
        saveBookingSnapshot?.(snapshot);
        setBookingDetails(normalized);
      } catch (error) {
        if (!isMounted) return;
        console.error('Lỗi tải booking:', error);
        setFetchError(error.message || 'Không thể tải thông tin booking.');
      } finally {
  ...
  const resolvePackageLabel = (pkgValue) => {
    if (!pkgValue) return '—';
    const preset = {
      daily: 'Daily',
      weekly: 'Weekly',
    ...
    const lower = String(pkgValue).toLowerCase();
    if (preset[lower]) return preset[lower];
    const matchedPackage = bookingState.packages?.find?.(pkg => String(pkg.id) === String(pkgValue));
    if (matchedPackage) {
      return matchedPackage.name || matchedPackage.display_name || matchedPackage.package_name || matchedPackage.code || `Package #${matchedPackage.id}`;
    }
    return pkgValue;
  };

  const effectiveBooking = bookingState.confirmedBooking || bookingDetails || storedSnapshot?.booking || null;
  const serviceType = effectiveBooking?.serviceType || bookingState.serviceType;
  const durationLabel = resolvePackageLabel(effectiveBooking?.packageDuration || bookingState.packageDuration);
  const priceDisplay = formatCurrency(effectiveBooking?.finalPrice ?? bookingState.paymentDetails.finalAmount);
  const paymentLabel = bookingState.lastPaymentLabel
    || storedSnapshot?.paymentMeta?.displayName
    || mapPaymentMethodLabel(bookingState.paymentMethod)
    || mapPaymentMethodLabel(storedSnapshot?.paymentMeta?.methodCode)
    || mapPaymentMethodLabel(bookingState.lastPaymentMethod);
  const seatLabel = serviceType === 'hot-desk'
    ? (effectiveBooking?.seatName || bookingState.selectedSeat?.name || 'Hot Desk Flex Access')
    : (effectiveBooking?.seatName || bookingState.selectedSeat?.name || 'Not selected');
  const displayDate = formatDate(effectiveBooking?.startDate || bookingState.date);
  const bookingReference = effectiveBooking?.bookingReference || bookingState.bookingReference;
  const showSuccessView = bookingState.bookingComplete || !!effectiveBooking;
      ...
      <ConfirmationContainer>
        {!showSuccessView ? (
          <>
            <BookingSummary>
              ...
              <SummaryRow>
                ...
                <SummaryValue>
                  {resolvePackageLabel(bookingState.packageDuration)}
                </SummaryValue>
              </SummaryRow>
              ...
              <SummaryRow>
                ...
                <SummaryValue>
                  {formatCurrency(bookingState.paymentDetails.finalAmount)}
                </SummaryValue>
              </SummaryRow>
              
              <SummaryRow>
                <SummaryLabel>Payment Method:</SummaryLabel>
                <SummaryValue>{mapPaymentMethodLabel(bookingState.paymentMethod)}</SummaryValue>
              </SummaryRow>
              ...
              <SummaryRow>
                ...
                <SummaryValue>
                  {bookingState.serviceType === 'hot-desk'
                    ? (bookingState.selectedSeat?.name || 'Hot Desk Flex Access')
                    : (bookingState.selectedSeat?.name || 'Not selected')}
                </SummaryValue>
              </SummaryRow>
            </BookingSummary>
          ...
          <>
            {isLoading && (
              <ImportantNote>
                <FaSpinner style={{ marginRight: '0.5rem' }} /> Đang tải thông tin booking mới nhất...
              </ImportantNote>
            )}
            {fetchError && (
              <ImportantNote style={{ borderLeftColor: '#e74c3c', color: '#c0392b' }}>
                {fetchError}
              </ImportantNote>
            )}
            <SuccessMessage>
            ...
            {bookingReference && (
              <BookingReference>
                ...
                <ReferenceNumber>{bookingReference}</ReferenceNumber>
              </BookingReference>
            )}
