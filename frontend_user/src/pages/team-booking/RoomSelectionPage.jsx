...
const RoomSelectionPage = () => {
  ...
  const buildStatusMapFromRooms = (list = []) => {
    const next = {};
    list.forEach((room) => {
      const code = room.roomNumber || room.room_code;
      if (!code) return;
      const backendStatus = (room.status || '').toString();
      const normalized = backendStatus.toLowerCase();
      let label = 'Available';
      if (room.activeBooking || room.available === false) {
        label = 'Occupied';
      } else if (normalized === 'maintenance' || normalized === 'disabled') {
        label = 'Maintenance';
  ...
  const buildRangeParams = () => {
    ...
    if (normalizedStartDate) {
      ...
      params.set('endDate', endDate || normalizedStartDate);
    }
    const normalizedStartTime = (startTime && startTime.trim()) ? startTime : '09:00';
    const normalizedEndTime = (endTime && endTime.trim()) ? endTime : (startTime && startTime.trim() ? startTime : '17:00');
    params.set('startTime', normalizedStartTime);
    params.set('endTime', normalizedEndTime);
    return params;
  };
    ...
    if (selectedService?.name === 'Meeting Room' || selectedService?.name === 'Private Office' || selectedService?.name === 'Networking Space'){
      const fetchStatuses = async () => {
        try {
          ...
          const res = await fetch(`${base}/api/team/services/${svc}/rooms/status${query ? `?${query}` : ''}`);
          ...
          if (data?.success && Array.isArray(data.data)){
            ...
            if (currentSelection && map[currentSelection.roomNumber] && map[currentSelection.roomNumber] !== 'Available'){
  ...
  }, [selectedService, selectedPackage, startDate, endDate, startTime, endTime, navigate]);
  ...
  const fetchAvailableRooms = async () => {
    try {
      ...
      if (data.success) {
        ...
        // Sắp xếp tự nhiên theo roomNumber (chữ cái + số) để ổn định thứ tự
        const rx = /^([A-Za-z]+)(\d+)$/;
        list = list.slice().sort((a,b)=>{
          const ac = a.roomNumber || a.room_code || '';
          const bc = b.roomNumber || b.room_code || '';
          const am = rx.exec(ac); const bm = rx.exec(bc);
          if (am && bm && am[1] === bm[1]) return parseInt(am[2]) - parseInt(bm[2]);
          return ac.localeCompare(bc, undefined, {numeric:true,sensitivity:'base'});
        });
        let roomsToRender = list;
        if (list.length === 0) {
        ...
        setRooms(roomsToRender);
        const immediateStatusMap = buildStatusMapFromRooms(roomsToRender);
        if (Object.keys(immediateStatusMap).length) {
          setStatusMap((prev) => {
            const next = { ...prev };
            Object.entries(immediateStatusMap).forEach(([code, status]) => {
              if (!prev[code] || status !== 'Available') {
                next[code] = status;
              }
            });
            return next;
      ...
      const filteredFallback = fallbackRooms.filter(r => r.name.toLowerCase().includes(selectedService.name.split(' ')[0].toLowerCase()) || selectedService.name === 'Networking Space');
      setRooms(filteredFallback);
      const fallbackStatusMap = buildStatusMapFromRooms(filteredFallback);
      if (Object.keys(fallbackStatusMap).length) {
        setStatusMap((prev) => {
          const next = { ...prev };
          Object.entries(fallbackStatusMap).forEach(([code, status]) => {
            if (!prev[code] || status !== 'Available') {
              next[code] = status;
            }
          });
  ...
  const resolveRoomStatus = (room) => {
    if (!room) return 'Available';
    const code = room.roomNumber;
    const override = code ? statusMap?.[code] : null;
    if (override) return override;
    if (room.available === false) return 'Occupied';
    if (room.status && room.status !== 'Available') return room.status;
    return 'Available';
  };

  const handleRoomSelect = (room) => {
    const st = resolveRoomStatus(room);
    if (st && st !== 'Available'){
      return;
    }
    setInfoMsg('');
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      if (!data.available){
        setStatusMap((prev) => ({
          ...prev,
          [selectedRoom.roomNumber]: 'Occupied'
        }));
        selectedRoomRef.current = null;
        setSelectedRoom(null);
        setInfoMsg('');
  ...
  return (
    <>
      ...
      <Container>
        ...
        <BookingInfo>
          ...
          <InfoCard>
            ...
            <InfoValue>{formatDate(displayStartDate || startDate)}</InfoValue>
          </InfoCard>
          <InfoCard>
      ...
      {(selectedService.name === 'Meeting Room' || selectedService.name === 'Private Office') ? (
        // Dynamic hiển thị danh sách phòng theo dữ liệu backend, bỏ cố định M1..M4 / P1
        <>
          <MapContainer>
            <MapImageWrapper>
              <img
            ...
            <LegendContainer>
              <LegendItem><span style={{background:'#3498db'}}></span><span>Occupied</span></LegendItem>
              <LegendItem><span style={{background:'#d1d5db'}}></span><span>Available</span></LegendItem>
              <LegendItem><span style={{background:'#e74c3c'}}></span><span>Maintenance</span></LegendItem>
            </LegendContainer>

            <RoomButtonsGrid>
              {rooms.map(r => {
                const code = r.roomNumber;
                const statusLabel = resolveRoomStatus(r);
                const disabled = statusLabel !== 'Available';
                ...
                return (
                  ...
                  >
                    ...
                    <RoomCapacity>Capacity: {r.capacity ?? '—'}</RoomCapacity>
                  </RoomButton>
                );
              })}
            </RoomButtonsGrid>
          ...
          {infoMsg && <EmptyMessage style={{maxWidth:800}}>{infoMsg}</EmptyMessage>}
          {rooms.length === 0 && !infoMsg && <EmptyMessage style={{maxWidth:800}}>Không có phòng khả dụng.</EmptyMessage>}
          {selectedRoom && (
            <ContinueSection>
              <SelectedRoomInfo>
        ...
        // Dynamic hiển thị Networking Space (Floor 3) thay vì cố định N1..N3
        <>
          <MapContainer>
            <MapImageWrapper>
              <img
          ...
          {infoMsg && <EmptyMessage style={{maxWidth:800}}>{infoMsg}</EmptyMessage>}
          {rooms.length === 0 && !infoMsg && <EmptyMessage style={{maxWidth:800}}>Không có không gian khả dụng.</EmptyMessage>}
          {selectedRoom && (
            <ContinueSection>
              <SelectedRoomInfo>
                <SelectedRoomName>Selected: {selectedRoom.name}</SelectedRoomName>
