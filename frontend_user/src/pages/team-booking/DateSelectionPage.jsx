...
const DateSelectionPage = () => {
  ...
  const handleContinue = () => {
    ...
    const startDateDisplay = formatDateOnly(start);
    const endDateDisplay = formatDateOnly(end);
    const startDateForApi = startDateDisplay;
    const endDateForApi = endDateDisplay;
                ...
                <div style={{ padding: '1rem', backgroundColor: '#f9f9f9', borderRadius: '4px' }}>
                  {endDateObj && selectedTime ? (
                    <div style={{ fontWeight: 500, color: '#45bf55' }}>
                      {formatDate(endDateObj)} at {`${String(endDateObj.getHours()).padStart(2,'0')}:${String(endDateObj.getMinutes()).padStart(2,'0')}`}
                    </div>
                  ) : (
