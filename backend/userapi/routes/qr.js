...
// Verify QR (requires auth to ensure owner-only access)
router.post('/verify', authMw, async (req, res) => {
  try {
    ...
    const verification = await qrService.verifyQRCode(qrCode, req.user.userId);
...
// Process check-in (requires auth)
router.post('/checkin', authMw, async (req, res) => {
  try {
    ...
    const isAdminActor = req.user && (req.user.role === 'admin' || req.user.role === 'superadmin');
    const result = await qrService.processCheckIn(
      qrCode,
      fullDeviceInfo,
      location,
      {
        ...
        role: req.user.role,
        bypassOwnership: Boolean(isAdminActor)
      }
    );
...
// Upload QR image and extract/verify (requires auth)
router.post('/upload', authMw, (req, res) => {
  uploadQRImage(req, res, async (err) => {
    ...
    try {
      ...
      const extractResult = await qrImageService.extractQRFromImage(filePath);
      if (extractResult.success && extractResult.qrString) {
        const verifyResult = await qrService.verifyQRCode(extractResult.qrString, req.user.userId);
      ...
      if (filePath) {
        try {
          ...
          console.warn('Could not remove uploaded file:', cleanupError.message);
        }
      }
    }
  });
...
// Upload QR image and process check-in (requires auth)
router.post('/upload-checkin', authMw, (req, res) => {
  uploadQRImage(req, res, async (err) => {
    if (err) return handleUploadError(err, req, res);
    const filePath = req.file?.path;
    try {
      ...
      const locationPayload = req.body.location ? JSON.parse(req.body.location) : null;
      const isAdminActor = req.user && (req.user.role === 'admin' || req.user.role === 'superadmin');
      ...
      console.error('QR upload check-in error:', error);
      res.status(500).json({ success: false, message: 'Server error while processing uploaded QR image' });
    } finally {
      if (filePath) {
        try {
