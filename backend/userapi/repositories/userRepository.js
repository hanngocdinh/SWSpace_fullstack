...
function createPgRepo() {
  ...
  return {
    ...
    async findByEmail(email) {
      const { rows } = await pool.query(
        'SELECT id, username, email, full_name, role, phone, status, created_at, last_login FROM users WHERE email = $1 LIMIT 1',
    ...
    async createResetToken(email, tokenHash, expiresAt) {
      const expiresAtValue = expiresAt instanceof Date ? expiresAt.toISOString() : expiresAt;
      await pool.query(
        'UPDATE users SET reset_password_token_hash=$1, reset_password_expires_at=$2 WHERE email=$3',
        [tokenHash, expiresAtValue, String(email || '').toLowerCase()]
      );
      return true;
    },
    async resetPasswordWithToken(tokenHash, newPassword) {
      const salt = await bcrypt.genSalt(10);
      const hash = await bcrypt.hash(newPassword, salt);
      const { rows } = await pool.query(
        'UPDATE users SET password_hash=$1, reset_password_token_hash=NULL, reset_password_expires_at=NULL WHERE reset_password_token_hash=$2 AND reset_password_expires_at > NOW() RETURNING id',
        [hash, tokenHash]
      );
