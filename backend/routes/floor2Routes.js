...
let floor2Activities = []; // {message, at}
...
function startWorkerIfNeeded() {
  ...
  const args = [
    ...
    '--draw-boxes', '1',
    '--max-dim', '640',
    '--jpeg-quality', '50'         // giảm chất lượng để cải thiện độ trễ
  ];
  // Strict human filtering heuristics enabled by env toggle (default on)
...
function pushActivity(message, at) {
  ...
  floor2Activities.unshift({ message, at: occurredAt });
  if (floor2Activities.length > 50) floor2Activities.pop();
  sseBroadcast('ai.activity', { message, at: occurredAt });
  floorActivityFeed.recordFloorEvent({ floor: 'floor2', source: 'ai', message, occurredAt });
}

// ----------- Status mapping (reuse seats enum) -----------
const toUI = (dbStatus) => {
  switch (dbStatus) {
...
async function getFloor2ZoneIds() {
...
// ----------- List rooms (admin + user) -----------
router.get('/rooms', async (req, res, next) => {
  try {
    const zones = await getFloor2ZoneIds();
...
router.post('/rooms', verifyToken, requireAdmin, async (req, res, next) => {
  try {
    const { roomCode, capacity, zoneId, roomType } = req.body || {};
    ...
    const zones = await getFloor2ZoneIds();
    if (!zones.length) return res.status(400).json({ error: 'No zones found for Floor 2' });
    let targetZoneId = zoneId ? parseInt(zoneId, 10) : undefined;
    if (!targetZoneId) {
      const type = String(roomType || '').toLowerCase();
      const desired = type === 'meeting room' || type === 'meeting_room' ? 'meeting_room'
                    : type === 'private office' || type === 'private_office' ? 'private_office'
                    : null;
      if (!desired) return res.status(400).json({ error: 'roomType required when zoneId not provided' });
      const match = zones.find(z => z.service_code === desired);
      if (!match) return res.status(400).json({ error: 'Matching zone for roomType not found' });
      targetZoneId = match.zone_id;
    }
    if (!zones.some(z => z.zone_id === targetZoneId)) return res.status(400).json({ error: 'Invalid zoneId for Floor 2' });
...
// Delete room (prevent if active bookings referencing it via seat_id? rooms may have separate bookings) - simple check
router.delete('/rooms/:roomCode', verifyToken, requireAdmin, async (req, res, next) => {
  try {
    ...
    // If bookings table contains room usage (assuming seat_id only for now) we skip advanced check
...
router.post('/ai/status', async (req, res, next) => {
  try {
    ...
    const prev = floor2AI.peopleCount;
    floor2AI.peopleCount = peopleCount;
    floor2AI.lastUpdate = detectedAt;
    if (floor2AI.active) sseBroadcast('ai.people', { peopleCount, detectedAt, boxes, frame });
    // Activity message logic
    if (peopleCount > 0) {
      pushActivity(`People detected: ${peopleCount}`, detectedAt);
    } else if (prev > 0 && peopleCount === 0) {
      pushActivity('Floor 2 is now empty', detectedAt);
    }
...
router.get('/ai/activities', (req, res) => {
  res.json({ items: floor2Activities });
});
